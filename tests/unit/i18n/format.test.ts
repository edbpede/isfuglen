...
describe("dates", () => {
  test("Danish full, long and short forms", () => {
    expect(formatDate(DATE, "da", "full")).toBe("fredag den 14. august 2026");
  ...
  test("English full, long and short forms", () => {
    expect(formatDate(DATE, "en", "full")).toBe("Friday 14 August 2026");
...
describe("meta line", () => {
  test("joins the parts that exist and drops the ones that do not", () => {
    ...
    ).toBe("fredag den 14. august 2026 · kl.\u00A015.30\u201317.00 · Lærerværelset");

    expect(formatMetaLine({ date: DATE }, "en")).toBe("Friday 14 August 2026");
